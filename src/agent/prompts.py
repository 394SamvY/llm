...
在得到五步分析结果后，请按以下优先级进行综合判定：

### 优先级1：训式明确指示（最高权重）
- 如果"读为"、"读曰"、"当为"等术语出现 → **假借说明**（置信度≥0.95）
- 如果"犹"、"谓之"、"之言"等术语出现 → 倾向**语义解释**（结合其他信息判断）
- 如果"A通/古字通"等同源术语 → **语义解释**（置信度≥0.90）

### 优先级2：文献佐证（第二权重）
- 有明确的异文或假借记录 + 义远 + 音近 → **假借说明**（置信度≥0.90）
- 无异文但有语源关联证据 + 义近 → **语义解释**（置信度≥0.85）

### 优先级3：语义-音韵结合（核心判断）
- 义远 + 音近 + (有异文或训式暗示) → **假借说明**
- 义近 + 音近 + (同源词特征或训式暗示) → **语义解释**
- 特殊情况：双声叠韵 + 义远 + 无明确假借术语 → 考虑**以声通义**（倾向语义解释）

### 优先级4：语境支撑（补充证据）
- 借字本义代入不通顺 + 正字本义通顺 → 支持**假借说明**
- 两字本义都通顺或都不通 → 需结合前三步判断
...
### 特殊情况处理
- "之为言/之言"格式：优先判为语义解释，但需检查异文和语境
- 同源词识别（声符关系、得声字）：即使义远也可能是语义解释
- 基本格式"A，B也"无明确术语：权衡所有因素后给出判断
